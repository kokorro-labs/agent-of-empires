...
use crate::docker::{
    self, ContainerConfig, DockerContainer, VolumeMount, CLAUDE_AUTH_VOLUME, GEMINI_AUTH_VOLUME,
...
/// - `start()` for command construction (Claude uses CLI flag, Gemini uses --yolo flag)
/// - `build_container_config()` for environment variables (OpenCode uses env var)
pub const YOLO_SUPPORTED_TOOLS: &[&str] = &["claude", "opencode", "gemini"];
